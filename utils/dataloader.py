...
def load_data(cfg, in_len=20, debug=False):
    ...
    data["timedata"] = np.array(TimeData)
...
class MazdaDataset(Dataset):
    ...
    def __getitem__(self, index):
        return {
            ...
            "timedata" : torch.Tensor(self.data["timedata"][index]),
        }
...
def create_dataset(cfg, original_data, index_list, is_train, mean_list=[], std_list=[]):
    ...
    for index in index_list:
        ...
        time_data_list.append(original_data["timedata"][index])
    ...
    data["timedata"] = rearrange(time_data_array, "a b c -> (a b) c")
