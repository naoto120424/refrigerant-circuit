...
def find_meanstd(train_index_list):
    ...
    input_data=[]
    for file in tqdm(train_csv_files):
...
def create_dataset(original_data, index_list, is_train, mean_list=[], std_list=[]):
    ...
    for i in tqdm(range(gt_array.shape[2])):
        gt_array[:, :, i] = (gt_array[:, :, i]-mean_list[i+spec_array.shape[2]])/std_list[i+spec_array.shape[2]]
    print("標準化処理終了")
    
    data['inp'] = rearrange(input_array, 'a b c d -> (a b) c d')    # shape(case_num*1179, 20, 39)
    data['spec'] = rearrange(spec_array, 'a b c -> (a b) c')        # shape(case_num*1179, 9)
    data['gt'] = rearrange(gt_array, 'a b c -> (a b) c')            # shape(case_num*1179, 30)
