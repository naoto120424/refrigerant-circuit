...
def main():
    ...
    with torch.no_grad():
        ...
        for id_b,batch in enumerate(test_dl):
            ...
            plt.plot(x, gt[0].cpu().detach().numpy(), color='dodgerblue', label = 'gt')
            plt.plot(x, pred[0].cpu().detach().numpy(), color='darkorange', label = 'pred')

            plt.xlabel('Crank Angle (deg)')
            plt.ylabel('Heat Release Rate (J/deg)')

            plt.xlim(320, 421)
            plt.ylim(0, max(gt[0].max().cpu().detach().numpy(), pred[0].max().cpu().detach().numpy()))

            plt.legend()

            plt.show()

            plt.savefig(os.path.join(vis_path, f"{batch['data_name'][0]}.png")) 
