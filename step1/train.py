...
from step1.utils import *
from step1.dataloader import *
from step1.visualization import *
...
def main():
    ...
    if not args.debug:
        train_index_list, test_index_list = train_test_split(np.arange(CFG.NUM_FIXED_DATA, len(data["inp"]) + CFG.NUM_FIXED_DATA), test_size=200)
        train_index_list, val_index_list = train_test_split(train_index_list, test_size=100)
    else:
        train_index_list, test_index_list = train_test_split(np.arange(CFG.NUM_FIXED_DATA, 100 + CFG.NUM_FIXED_DATA), test_size=10)
